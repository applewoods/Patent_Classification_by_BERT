...
class EngineForBert(MyEngine):
    ...
    def train(engine, mini_batch):
        ...
        engine.optimizer.zero_grad() # optimizer 초기화(initialization)

        x, y = mini_batch['input_ids'], mini_batch['labels']
        # engine.device => GPU로 데이터 옮김
        x, y = x.to(engine.device), y.to(engine.device)
        mask = mini_batch['attention_mask']
        mask = mask.to(engine.device)

        x = x[:, :engine.config.max_length]     # size: (n,l, 1) or (n, l)

        # Take feed-forward
        y_hat = engine.model(x, attention_mask=mask).logits     # model.logits => softmax 넣기 직전 결과값, size: (n, |c|)

        loss = engine.crit(y_hat, y)    # engine.crit => crossentropy loss
        loss.backward()
...
class BertTrainer(Trainer):
    ...
    def train(
        ...
        :param model: BERT 모델.
        :param crit: criterion(기준), loss함수.
        :param optimizer: 최적화 함수(adam, radam).
        :param scheduler: scheduler.
        :param train_loader: train_loader.
        :param valid_loader: valid_loader.
        '''
